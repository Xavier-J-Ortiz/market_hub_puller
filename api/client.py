...
from processing.constants import Futures_results
...
Url_json_headers = list[tuple[str, list[int], dict[str, str]]]


def create_post_futures(urls_json_headers: Url_json_headers) -> list[Future]:
    all_futures: list[Future] = []
    for url_json_header in urls_json_headers:
        url: str = url_json_header[0]
        ...
        ids: list[int] = url_json_header[1]
        header: dict[str, str] = url_json_header[2]
...
def futures_results(futures: list[Future]) -> Futures_results:
            ...
            if "x-esi-error-limit-remain" in result.headers:
                ...
                if error_limit_remaining != "100" and PRINT_INFORMATIONAL_ERR_LIMITS:
                    ...
                    print(
                        ...
                        f"the Error Limit Remaning: {error_limit_remaining} Limit-Rest "
