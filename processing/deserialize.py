...
def deserialize_order_item_p1(
    ...
    # `p1_deserialized_results` are the deserialized results of a first page of a
    #   request
...
def get_source_data(region: str, global_orders: GlobalOrders) -> None:
    ...
    # Fetches all orders in a region. regional_orders[region].all_orders_data looks
    #  like:
    #
    ...
    global_orders[region].all_orders_data = deserialize_order_items(
        region_name,
        [],
        u.create_all_order_url,
    )[0]
    # Fetches all Active order item IDs in a region extracted from
    #   regional_orders[region].all_orders_data fetched from line above,
